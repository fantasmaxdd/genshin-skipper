...
def check_and_skip():
    ...
    if option_pos:
        ...
        target_x = x + 350 + random.randint(-20, 20)
        target_y = y + random.randint(-4, 4)

        pydirectinput.moveTo(target_x, target_y, duration=0.05)
        time.sleep(0.04) 
        
        pydirectinput.click()
        print(f"[СКИППЕР] Кликнули по тексту диалога в точке ({target_x}, {target_y})")
        
        pydirectinput.moveTo(old_x, old_y, duration=0.03)
